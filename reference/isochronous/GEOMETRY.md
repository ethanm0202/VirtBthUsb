# Isochronous geometry reference

This document records the measured isochronous geometry matrix, packet counts, accepted combinations, and negative controls for the emulated USB test device.

## 1. SCO Interface Topology & Alternate Settings

In Bluetooth USB controllers (and the descriptors recorded in `reference/VIRTUAL-HCI-REFERENCE.txt`), Interface 1 carries synchronous voice (SCO/eSCO) traffic using isochronous endpoints:
...
| Alternate Setting | Maximum Bytes / Interval (`wMaxPacketSize`) |
| :---: | :---: |
| Alt 1 | 9 |
| Alt 2 | 17 |
| Alt 3 | 25 |
...
These are USB packet capacities, not PCM sample formats or codec frame sizes. The instrument measured transport geometry without an audio codec. For the Bluetooth driver's mSBC framing and pacing, see [Voice over the isochronous endpoints](../../docs/ARCHITECTURE.md#voice-over-the-isochronous-endpoints).
